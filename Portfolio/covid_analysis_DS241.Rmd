...
df_pop <- wb_data("SP.POP.TOTL") %>%
  filter(date==2019) %>%
  select(country,SP.POP.TOTL)
df_pop[df_pop=="United States"]<-"US"
df_pop[df_pop=="United Kingdom"]<-"UK"

country_data <- coronavirus_df %>%
...
We also need to take a note of the "as_of_date" for the data so that we can properly label our visualization.
...
Adding country population and covid cases normalized by population.

```{r}
country_data <- country_data %>%
  left_join(df_pop, by=c('country')) %>%
  mutate(
    covid_norm = (cumulative_cases/SP.POP.TOTL)*1000000
  )
```

Plotting cumulative cases per population size vs days elapsed since the 10th confirmed death in each country.
...
ggplot(data = country_data,
  ...
  geom_point(data = country_data %>% filter(end_date)) +
  # add country labels, nudged above the lines
  geom_label_repel(nudge_y = 1, direction = "y", hjust = .001) + 
  # turn off legend
  guides(color = FALSE) +
  # use pretty colors
  scale_color_viridis_d() +
  ...
  labs(
    ...
    title = "Cumulative deaths from COVID-19 Normalized by Population, select countries",
    subtitle = glue("Data as of", as_of_date_formatted, .sep = " "),
    caption = "Source: github.com/RamiKrispin/coronavirus"
  )
```